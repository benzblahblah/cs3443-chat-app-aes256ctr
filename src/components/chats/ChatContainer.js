...
export default class ChatContainer extends Component {
			...
			let newChats = chats.map((chat)=>{
				if(chat.id === chatId){
					...
					if(chat.isCommunity){
						...
						console.log('private')
						message.message = this.decrypt(message.message, chat.id)
						chat.messages.push(message)
					}
	...
	*/
	...
	// using aes-256-ctr algorithm for encryption/decryption
    algorithm = 'aes-256-ctr';

	// Take two paras algorithm and password -> room_id
	encrypt(text,key){
	...
	*   Separate the room by using boolean isCommunity if true the chat room is public.
	*   Generate key by using activeChat.id
	*/
	sendMessage = (chatId, message)=>{
		const { socket } = this.props
		if(this.state.activeChat.isCommunity === true){
			console.log('public')
			socket.emit(MESSAGE_SENT, {chatId, message} )
		}else{
			console.log('private')
			message = this.encrypt(message, this.state.activeChat.id)
			socket.emit(MESSAGE_SENT, {chatId, message} )
		}
	}
